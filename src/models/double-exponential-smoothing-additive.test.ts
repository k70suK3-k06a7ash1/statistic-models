...
test("doubleExponentialSmoothingAdditive should throw an error if alpha or beta is out of range", () => {
	expect(() =>
		doubleExponentialSmoothingAdditive([1, 2, 3], -0.1, 0.5),
	).toThrowError("Alpha and Beta must be between 0 and 1.");
	expect(() =>
		doubleExponentialSmoothingAdditive([1, 2, 3], 0.5, 1.1),
	).toThrowError("Alpha and Beta must be between 0 and 1.");
});

test("doubleExponentialSmoothingAdditive should handle array with single element", () => {
	const data = [10];
	const alpha = 0.5;
	const beta = 0.5;
...
test("doubleExponentialSmoothingAdditive should calculate level, trend and smoothed data correctly", () => {
...
test("doubleExponentialSmoothingAdditive forecast should return correct values", () => {
	const data = [10, 12, 14, 16, 18];
	const alpha = 0.5;
	const beta = 0.5;
	const result = doubleExponentialSmoothingAdditive(data, alpha, beta);
